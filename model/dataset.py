...
from model.chemtools import get_leaves
...
class MoleculeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return HierGraph.tensorize(self.batches[idx], self.vocab, self.avocab)

class MolEnumRootDataset(Dataset):

    def __init__(self, data, vocab, avocab, flag):
        self.batches = data
        self.vocab = vocab
        self.avocab = avocab
        self.flag = flag
    ...
    def __getitem__(self, idx):
        ...
        #smiles_list = set( [Chem.MolToSmiles(mol, rootedAtAtom=i, isomericSmiles=False) for i in mol.GetAtoms()] )
       
        smiles_list = sorted(list(smiles_list)) #To ensure reproducibility

        safe_list = []
        for s in smiles_list:
            hmol = HierGraph(s, self.flag)
            ok = True
        ...
        if len(safe_list) > 0:
            return HierGraph.tensorize(safe_list, self.vocab, self.avocab)
        else:
            return None

class MolDoubleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        datam = self.batches[idx].split(' ')
        #x, y = zip(*self.batches[idx])
        x = MolGraph.tensorize(datam[0], self.vocab, self.avocab) #no need of order for x
        y = MolGraph.tensorize(datam[1], self.vocab, self.avocab)
        return x + y
